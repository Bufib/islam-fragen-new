...
export default function AddNews() {
  ...
  const renderImageItem = ({
    ...
    item: { uri: string; base64: string | null };
  }) => {
    return (
      <View style={styles.imageContainer}>
        <Image source={{ uri: item.uri }} style={styles.imagePreview} />
  ...
  return (
    ...
    >
      {/* 
        A single FlatList to handle ALL scrolling:
        1) Form in the ListHeaderComponent
        2) selectedImages as the data
        3) Submit button in the ListFooterComponent 
      */}
      <FlatList
        style={[styles.scrollViewStyle, themeStyles.defaultBackgorundColor]}
        contentContainerStyle={styles.scrollViewContentContainer}
        ...
        // If you want the images in a horizontal row, add `horizontal`:
        // horizontal
        ListHeaderComponent={
          <>
            <ThemedText style={styles.header} type="title">
...
const styles = StyleSheet.create({
  ...
  imageContainer: {
    ...
    shadowRadius: 6,
    elevation: 2,
    marginBottom: 20, // so each image is spaced vertically
  },
  imagePreview: {
    width: 120,
    height: 120,
