...
//   // Set token for global push notification
//   useEffect(() => {
...
//       try {
//         const tokenStored = await AsyncStorage.getItem(TOKEN_STORAGE_KEY);
//         if (tokenStored) return;

//         const { error } = await supabase.from("user_token").
//         upsert({
//           user_id: userId,
...
//         await AsyncStorage.setItem(TOKEN_STORAGE_KEY, "true");
//       } catch (error) {
...
//     handleTokenStorage();
//   }, [expoPushToken, userId, useNotificationStore((state) => state.getNotifications)]);;
